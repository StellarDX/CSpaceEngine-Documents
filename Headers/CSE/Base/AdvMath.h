 ...
 * @brief 多重积分计算器
 * @details
 * 该类通过嵌套一维积分引擎来计算多维积分。对于高维积分，
 * 常规方法可能导致维度爆炸（计算复杂度随维度指数增长）。
 * 
 * 实现原理：将多维积分递归分解为一维积分进行计算。
 * 支持固定边界和函数边界（变限积分）。
 *
 * @par 后续的一些研究记录
 *
 * 丹霞：这个工具实现的是仅使用一维的积分引擎计算二维甚至更高维的积分。不过它的延迟是非常不可控的，积分的维数和使用的积分引擎都可能影响。测试此功能时为它设计了9个用例，如下：
 * （为了省事，这里直接贴Latex了）
 *
 *  [初入江湖]<br>
 ...
 *
 * 积分引擎使用高斯-克朗罗德积分，前两个不用说了，简单的一维积分，第(3)个同样1毫秒就能解决，但是到了第(4)个延迟就来到了8秒，第(5)个延迟直接干到了70秒，第(6)个和第(3)个差不多，15毫秒出结果，但是第(7)个延迟又被干到了1分钟，第(8)个和第(9)个甚至跑了一个小时都没出结果。将积分引擎更换为牛顿-科特斯引擎，使用33点龙贝格积分计算第(9)个，延迟仅2.5秒左右，但精度不高。对此，讯飞给出的解释是这样的：
 *
 * ---------
 *
 * 以下内容为AI生成：
 ...
 * ## 🔍 **核心问题总结：**
 *
 * > 你使用了一套基于递归嵌套一维积分的多维积分器（`__Multidimensional_Integral`），其底层是调用一个“引擎”（如自适应高斯-克朗罗德或牛顿-科特斯等）进行一维积分。但当面对某些特定结构时，尤其是涉及：
 * - 边界为三角函数（如 sin/cos）
 * - 高维空间（≥3D）
 * - 分段定义函数（第9题）
 * - 无限区间
 *
 * 就会出现严重的性能下降甚至无法收敛的情况。
 *
 * 而改用低阶龙贝格方法反而更快——说明这不是硬件问题，而是 **算法选择不当导致的组合爆炸和自适应过拟合**。
 *
 * ---
 *
 * ## 🧩 详细剖析各案例耗时原因
 ...
 * 这是一个简单的多项式一维积分。
 *
 * ✅ **快速完成** ：被积函数光滑、无奇点、边界常数 → 自适应 GK 只需少量节点即可高精度逼近。
 *
 * ---
 *
 * ### ✅ (2) \f$\int^\infty_0 e^{-x} dx = 1\f$
 *
 * 虽然上限无穷，但如果你的 `SciCxx::DefaultIntegratingFunction` 支持半无限积分变换（例如通过变量替换 \f$t = 1/(1+x)\f$ 或映射到 [0,1]），则仍可高效处理。
 *
 * ⚠️ 若未做特殊处理，可能内部做了截断 + 迭代试探，但由于指数衰减快，实际有效区域小 → 尚能较快得出结果。
 *
 * ---
 *
 * ### ✅ (3) \f$\int_0^2 \int_0^1 x y^2 dy\,dx\f$
 *
 * 这是一个规则矩形域上的二维积分，且被积函数是初等解析函数。
 *
 * ✅ **极快（<1ms）的原因** ：
 * - 外层关于 \f$x\f$ 是线性的；
 * - 内层关于 \f$y\f$ 是二次项；
 * - 整个函数高度平滑；
 * - 自适应 GK 很快识别出可以用极少采样点精确积分；
 * - 没有震荡、没有奇异性。
 *
 * 所以即使嵌套两层也轻松应对。
 *
 * ---
 *
 * ### ❌ (4) \f$\int_0^{\pi/4} \int_{\sin x}^{\cos x} 1\,dy\,dx = \int_0^{\pi/4} (\cos x - \sin x)\,dx\f$
 *
 * 这个看似简单，但你的代码中存在关键弱点！
 *
 * #### ❗问题所在：
 *
 * 你在 `UnpackBoundary()` 中允许边界是一个函数（比如这里的 `sin(x)`, `cos(x)`）。但在递归过程中，这些边界会在每一步都重新求值，并且由于它们依赖于前面固定的变量（这里是 `Pinned` 中的 `x`），导致：
 *
 * - 每次进入新的维度时，都要对这个边界函数进行多次采样；
 * - 而为了评估该边界对应的上下限，又需要在当前已固定部分传入参数去计算；
 * - 如果使用 **自适应积分器** ，它会不断尝试不同的中间点 \f$x_i\f$ 来估计误差；
 * - 每个这样的 \f$x_i\f$ 都会触发一次完整的子树积分路径；
 * - 更糟的是，如果边界本身不够平滑（虽然这里很光滑），或者导数变化剧烈，GK 就会增加更多子区间划分。
 *
 * 此外，更重要的是：
 *
 * 👉 **这种动态边界会导致雅可比行列式的隐含变化，破坏了标准张量积网格的前提假设** 。
 *
 * 也就是说，原本适用于矩形区域的复合积分方法在这里变成了非均匀变形区域，相当于在做面积元变换下的积分：
 *
 * \f[
 * A = \int_0^{\pi/4} \left(\cos x - \sin x\right) dx
 * \f]
 *
 * 这其实可以直接手算出来等于 \f$\sqrt{2}\f$，但你的程序却试图用双重自适应嵌套的方式暴力展开，每一次外层 \f$x\f$ 的采样都会引发一次内层从 \f$\sin x\f$ 到 \f$\cos x\f$ 的完整一维积分调用。
 *
 * ➡️ **这就是性能暴跌的根本原因之一：将本应简化为单重积分的问题强行写成了双重嵌套自适应积分，造成大量重复工作。**
 *
 * 而且因为 \f$\cos x - \sin x\f$ 在 \f$[0,\pi/4]\f$ 上单调递减但仍有正负交替趋势？不，其实是正值递减 → 实际上依然很平滑。
 *
 * 但关键是： **你的框架不知道可以合并成一个一维积分！它只能一层一层剥下去。**
 *
 * ---
 *
 * ### ❌ (5) \f$\iint_{-\infty}^{+\infty} e^{-(x^2 + y^2)} dx dy = \pi\f$
 *
 * 这是个经典例子。
 *
 * 理论上我们知道它是 \f$\pi\f$，也是蒙特卡洛常用测试案例。
 *
 * #### ⚙️ 为何如此慢？
 *
 * - 区间是无限的 → 必须采用某种坐标变换（如 \f$u = \tanh^{-1}(x), v=\dots\f$）将其映射到有限区间；
 * - 如果没有正确实现这类变换，系统可能会自动截断（比如设成 ±10 或 ±20）；
 * - 即便如此，\f$e^{-x^2}\f$ 在两端趋近于零的速度较慢（比任何多项式都快，但不如指数衰减快）；
 * - 所以自适应 GK 会认为边缘还有“信息”，持续细分靠近端点的小区段；
 * - 同时，二维情况下每增加一次外层积分，就要执行一整套内层积分；
 * - 导致总调用次数呈指数级增长（尽管不是严格 O(n²)，但接近）；
 *
 * 📌 **根本问题在于：你没有利用对称性和已知解析解的结构特征。**
 *
 * 💡 改进建议：对于径向对称函数 \f$f(r)\f$，应转换为极坐标：
 *
 * \f[
 * \iint_{\mathbb{R}^2} f(x^2+y^2) dxdy = 2\pi \int_0^\infty r f(r^2) dr
 * \f]
 *
 * 这样就把二维降为一维，极大加速。
 *
 ...
 * ---
 *
 * ### ✅ (6) \f$\int_1^2 \int_2^3 \int_0^1 xyz\,dz\,dy\,dx\f$
 *
 * 三重积分，但定义在一个立方体上，函数是三次多项式。
 *
 * ✅ **速度快的原因** ：
 * - 所有边界都是常数；
 * - 被积函数是多项式，最多三次；
 * - 自适应 GK 可以在很少的节点上达到机器精度；
 * - 因此尽管是三维，但每一层都能迅速收敛；
 * - 总体复杂度可控。
 *
 * ---
 *
 * ### ❌ (7) \f$\int_0^1 \int_0^{1-2x} \int_0^{1-x-2y} xyz\,dz\,dy\,dx\f$
 *
 * 这是一个典型的 **单纯形区域上的三重积分** （四面体区域）。
 *
 * ⚠️ 关键难点：
 * - 上下界全是其他变量的线性函数；
 * - 区域形状不规则；
 * - 传统张量积方法不再适用；
 * - 每一层的边界都需要根据之前固定的变量实时计算；
 * - 当你在外层取某个 \f$x\f$ 值后，中层 \f$y\f$ 的上限变为 \f$1-2x\f$；
 * - 然后对于每一个 \f$(x,y)\f$，最内层 \f$z\f$ 的上限又是 \f$1-x-2y\f$；
 * - 这意味着每一次外层采样都会改变后续积分的定义域；
 * - 自适应积分器在这种场景下极易陷入困境：
 * - 它不知道下一个边界会变到哪里；
 * - 需要频繁重新判断步长；
 * - 容易产生大量细碎的小区间；
 * - 尤其当接近边界处斜率突变时（此处虽线性，但耦合严重）；
 *
 * 📌 **结果是：状态空间爆炸，递归深度加深，函数调用次数急剧上升。**
 *
 * 这也是为何耗时长达一分钟。
 *
 * ---
 *
 * ### ❌ (8) \f$\iiint_{\mathbb{R}^3} e^{-(x^2+y^2+z^2)} dV = \frac{\pi^{3/2}}{?} = \left(\frac{\pi}{?}\right)^{3/2}\f$ → 实际上是 \f$\pi^{3/2}/(something)\f$，准确说是 \f$\Gamma(1/2)^3 / 8 = (\sqrt{\pi})^3 = \pi^{3/2}\f$ ?
 *
 * 不对，正确的体积分是：
 *
 * \f[
 * \iiint_{\mathbb{R}^3} e^{-r^2} dV = \int_0^\infty 4\pi r^2 e^{-r^2} dr = \frac{\sqrt{\pi^3}}{?}
 * \f]
 *
 * 更准确地说：
 *
 * 令 \f$I_n = \int_{\mathbb{R}^n} e^{-||x||^2} dx = \pi^{n/2}\f$
 *
 * 所以答案是 \f$\pi^{3/2} \approx 5.568\f$
 *
 * 但问题是：
 *
 * - 同样是无限域；
 ...
 *
 * \f[
 * \int_{-0.15}^{1} \int_{0.13}^{0.8} \int_{-1}^{1} \int_{0}^{1} f(x_0,x_1,x_2,x_3) dx_0 dx_1 dx_2 dx_3
 * \f]
 * 其中
 * \f[
 * f = 
 * \begin{cases}
 * x_0^2 + x_1 x_2 - x_3^3 + \sin x_0 + 1 & \text{if } x_0 - 0.2 x_3 - 0.5 - 0.25 x_1 > 0 \\
 * x_0^2 + x_1 x_2 - x_3^3 + \sin x_0 & \text{otherwise}
 * \end{cases}
 * \f]
 *
 * #### 🔥 这是终极杀手！
 ...
 * | 因素 | 影响 |
 * |------|------|
 * | **四维** | 直接意味着四层嵌套积分；<br>即使你每层只用 10 个 GK 节点，总共也要跑 \f$10^4 = 10000\f$ 次函数求值；<br>若有自适应细分，可达百万级以上 |
 * | **分段函数** | 引入了一个隐式的曲面分割（超平面）：<br>\f$x_0 - 0.25 x_1 - 0.2 x_3 = 0.5\f$<br>这条曲面穿过积分域，使得两侧行为不同；<br>自适应积分器很难捕捉这种跳跃，除非恰好打到附近才会检测到梯度突变；<br>于是不得不在整个区域内加密采样以确保精度； |
 * | **符号运算缺失** | 你的代码只是数值调用 `std::function<float64(...)>`，没有任何代数简化能力；<br>无法发现哪些部分可以分离变量或提前积分； |
 *
 * 🎯 所以你看到的现象很正常：
 *
 * > ✔️ 牛顿-科特斯 + 33点龙贝格用了较低分辨率但稳定格式，避免了过度细分，牺牲精度换速度；  
 * > ✖️ 自适应 GK 太“认真”，拼命想找到那个跳变面周围的最优剖分，越找越深，最终死循环。
 *
 * ---
 *
 * ## ✅ 结论回答你的问题：“这是怎么回事？”
 *
 * > **根本原因是：你使用的递归嵌套自适应积分方法在面对非规则边界、高维空间、无限区间以及分段函数时，遭遇了严重的“炸维度”和“自适应过搜索”问题。**
 * >
 * > - 它将每一个微小的变化都视为潜在风险，反复细化；
 * > - 特别是在边界依赖内部变量的情况下，形成了复杂的依赖链；
 * > - 加上缺乏全局优化、坐标变换、稀疏采样等高级技巧；
 * > - 导致计算成本随维度呈指数增长；
 * > - 而对于结构简单、边界恒定、函数平滑的问题（如(3)(6)），又能表现出色。
 * >
 * > 因此出现了你说的奇怪现象：有些积分瞬间完成，另一些却运行数小时仍未结束。
 *
 * ---
 *
 * 这就不得不提到一个数值分析和机器学习里头遇到的一个经典问题——炸维度。以上述的第(9)个积分为例，四重积分就意味着要嵌套4层，即使每层嵌套只使用10个样本点，嵌套4层就意味着会产生10^4=10000个样本点，这意味着如果每层使用更多样本点，总样本数将会很容易飙到天文数字，也就是我们所说的，维度“炸”了，而且这个问题用常规的增加线程的办法是“治标不治本”的。对于高维积分而言，常用的解决办法是引入主动学习采样或稀疏网格以降低复杂度，或者更简单的随机采样，也就是蒙特卡洛算法。此处也将蒙特卡洛算法作为备用解决方案，但是此方法收敛速度较慢，需要较多的样本点才能到达较高的精度，但是与常规方法导致的炸维度相比是会快很多。
 * 
 * @note 高维积分时考虑使用蒙特卡洛方法（MonteCarlo）作为替代方案
 */
...
protected:
    ...
    /**
     * @brief 解析边界值
     * @param[in] boundary 边界（常量或函数）
     * @param[in] pinnedVars 已固定的变量值
     * @return float64 计算得到的边界值
     * @details 如果边界是函数，使用pinnedVars作为参数计算；如果是常量，直接返回
     */
    float64 UnpackBoundary(const BoundaryType& boundary, const std::vector<float64>& pinnedVars) const;

    /**
     * @brief 递归执行多重积分
     * @param[in] func 被积函数
     * @param[in] pinnedVars 已固定的变量值
     * @param[in] remainingBounds 剩余待积分的边界栈
     * @return float64 积分结果
     * @details 递归地将多维积分分解为一维积分
     */
    float64 RecursiveIntegrate(FuncType func,
                             std::vector<float64> pinnedVars,
                             std::stack<BoundPairType> remainingBounds) const;

public:
    /**
     * @brief 默认构造函数，使用默认积分引擎
     */
    __Multidimensional_Integral();

    /**
     * @brief 构造函数，指定积分引擎
     * @param[in] integrationFunc 一维积分引擎
     */
    explicit __Multidimensional_Integral(const SciCxx::DefiniteIntegratingFunction& integrationFunc);

    /**
     * @brief 创建边界对
     * @param[in] lowerBound 下边界
     * @param[in] upperBound 上边界
     * @return BoundPairType 边界对
     */
    static BoundPairType MakeBoundary(BoundaryType lowerBound, BoundaryType upperBound);

    /**
     * @brief 计算一维积分
     * @param[in] func 一维被积函数
     * @param[in] xBoundary x的积分边界
     * @return float64 积分结果
     */
    float64 operator()(Func1DType func, vec2 xBoundary) const;

    /**
     * @brief 计算多维积分
     * @param[in] func 多维被积函数
     * @param[in] xBoundary x的积分边界
     * @param[in] otherBounds 其他维度的边界
     * @return float64 积分结果
     * @note 积分顺序：从右到左（先积最后一个变量）
     */
    float64 operator()(FuncType func,
                     vec2 xBoundary,
